"""Scraper for neonichiban.com comic listings."""
...
class NeonIchibanScraper(BaseScraper):
    """Scrape comic listings from neonichiban.com.

    Neon Ichiban is a digital comics platform run by ex-Comixology staff.
    This scraper extracts cover images, titles, and descriptions from
    their public-facing catalog pages.
    """

    source_name = "neonichiban"
    base_url = "https://neonichiban.com"
    ...
    def scrape_listings(self, max_items: int = 50) -> List[ScrapedComic]:
        """Scrape comic listings from the Neon Ichiban catalog.
        ...
        try:
            ...
            logger.warning(
                "beautifulsoup4 not installed; cannot scrape neonichiban.com"
            )
            return comics
        ...
        try:
            resp = session.get(
                urljoin(self.base_url, "/browse"), timeout=30
            )
            resp.raise_for_status()
        except Exception as e:
            logger.error("Failed to fetch Neon Ichiban listings: %s", e)
            return comics

        soup = BeautifulSoup(resp.text, "html.parser")

        # Look for comic cards/items - adapt selectors to site structure
        for card in soup.select(
            "a[href*='/comic'], a[href*='/series'], .comic-card, .series-card"
        )[:max_items]:
            try:
    ...
    def scrape_comic_detail(self, url: str) -> Optional[ScrapedComic]:
        ...
        Args:
            url: URL of the comic detail page on neonichiban.com.
